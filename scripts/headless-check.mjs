...
try {
  ...
  const probes = [
    ...
    ["关键 DOM 齐全", '["rememberKeys","uploadZone","inputText","rewriteBtn","proxyStatus","headerInfo","progressWrap","viewTabs","compareArea","detectPanelBody"].filter(id=>!document.getElementById(id)).join(",")',
      (v) => v === ""],
    ["核心函数均已定义", '["startRewrite","singleRewrite","batchRewrite","doRewrite","splitIntoChunks","estimateMaxTokens","buildMessages","checkProxyHealth","migrateLegacyKeys"].filter(f=>typeof window[f]!=="function").join(",")',
      (v) => v === ""],
