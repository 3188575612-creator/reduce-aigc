...
| `npm run test:ui` | 无头 Edge / Chrome + CDP：在真实页面上下文断言分段、tokenize 粒度、质量自检、模型增删与迁移、草稿、无障碍、玻璃层与令牌、推理预算、**左右两栏逐层对齐**等 55 项，并抓运行时异常 |
...
| 能做什么 | 说明 |
|---|---|
| 🔍 质量自检 | 自动比对**技术信息是否丢失**（版本号 / 表名 / 参数名 / 百分比）与**改写幅度**（相似度过高或过低都会提示） |
